...
def get_resnet_output(predictor, data_batch, data_names):
    ...
    if output_all[0].has_key('conv_embed_output'):
        feat = output_all[0]['conv_embed_output'].copy()
    else:
        feat = None
    return data_dict_all[0]['data'], feat
...
def im_detect(predictor, data_batch, data_names, scales, cfg):
    ...
    if output_all[0].has_key('aggr_feat_output'):
        aggr_feat = output_all[0]['aggr_feat_output'].copy()
    else:
        aggr_feat = None

    return zip(scores_all, pred_boxes_all, data_dict_all), aggr_feat, weights
...
def pred_eval(gpu_id, feat_predictors, aggr_predictors, test_data, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True):
    ...
    strided_idx = [_i for _i in range(all_frame_interval) if _i % cfg.TEST.sample_stride == 0]
    print(strided_idx, len(strided_idx))
    print('all_frame_interval', all_frame_interval, 'sample stride', cfg.TEST.sample_stride)
    ...
    for frame_offset, img_path, im_info, key_frame_flag, data_batch in test_data:
        ...
        if key_frame_flag == 0:
            ...
            while len(data_list) < cfg.TEST.KEY_FRAME_INTERVAL * cfg.TEST.sample_stride +1:
            ...
            if cfg.TEST.KEY_FRAME_INTERVAL == 0:
                ...
                pred_result, aggr_feat, nonlocal_weights = im_detect(aggr_predictors, data_batch, data_names, scales, cfg)
                ...
                if vis:
                    output_dir = os.path.join(imdb.result_path, *img_paths_list[cfg.TEST.KEY_FRAME_INTERVAL * cfg.TEST.sample_stride])
        ...
        #################################################
            ...
            scales = [iim_info[0, 2] for iim_info in im_info]
            while end_counter < min(roidb_frame_seg_len, cfg.TEST.KEY_FRAME_INTERVAL * cfg.TEST.sample_stride + 1):
                data_list.append(image)
                feat_list.append(feat)
                img_paths_list.append(img_path)
                frame_offset_list.append(frame_offset)
                ...
                strided_feat_list = [feat_list[_idx] for _idx in strided_idx]
                prepare_data(strided_data_list, strided_feat_list, data_batch)
                pred_result, aggr_feat, _ = im_detect(aggr_predictors, data_batch, data_names, scales, cfg)
...
def pred_eval_recur(gpu_id, feat_predictors, aggr_predictors, test_data, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True):
    """
    wrapper for calculating recursive online validation for faster data analysis
    in this example, all threshold are set by hand
    :param predictor: Predictor
    :param test_data: data iterator, must be non-shuffle
    :param imdb: image database
    ...
    # loop through all the test data
    for image_path, im_info, key_frame_flag, data_batch in test_data:
        t1 = time.time() - t
        t = time.time()
        ...
        if key_frame_flag == 0:
            ...
            while len(data_list) < all_frame_interval:
                data_list.append(image)
                feat_list.append(feat)
...
def pred_eval_multiprocess(gpu_num, key_predictors, cur_predictors, test_datas, imdb, cfg, vis=False, thresh=1e-3, logger=None, ignore_cache=True):
    ...
    if cfg.TEST.SEQ_NMS==False:
        ...
        for job in jobs:
            res.append(job.get())
        info_str = imdb.do_python_eval_gen_gpuid(gpu_num)
    if logger:
...
def draw_all_detection(im_array, detections, class_names, scale, cfg, threshold=0.1):
    ...
    for j, name in enumerate(class_names):
        ...
        for det in dets:
            ...
            box_coords = (
            (bbox[0], bbox[1] + 30), (bbox[0] + text_width - 2, bbox[1] + 30 - text_height -2))
            ...
            cv2.putText(im, '%s %.3f' % (class_names[j], score), (bbox[0], bbox[1] + 30),
                        color=color_white, fontFace=cv2.FONT_HERSHEY_COMPLEX, fontScale=font_scale, thickness=2)
    return im
...
def process_pred_result(pred_result, imdb, thresh, cfg, nms, all_boxes, idx, max_per_image, vis, center_image, scales):
    for delta, (scores, boxes, data_dict) in enumerate(pred_result):
        ...
        out_im = draw_all_detection(center_image, boxes_this_image, imdb.classes, scales[delta], cfg)
        if vis:
            boxes_this_image = [[]] + [all_boxes[j][idx + delta] for j in range(1, imdb.num_classes)]
            out_im = draw_all_detection(center_image, boxes_this_image, imdb.classes, scales[delta], cfg)
            return out_im
