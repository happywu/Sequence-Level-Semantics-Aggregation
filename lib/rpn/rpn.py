...
def get_rpn_triple_batch(roidb, cfg):
    ...
    # gt boxes: (x1, y1, x2, y2, cls)
    # print('gt classes size', roidb[0]['gt_classes'].size)
    if roidb[0]['gt_classes'].size > 0:
        gt_inds = np.where(roidb[0]['gt_classes'] != 0)[0]
        # print('gt', roidb[0]['gt_classes'], gt_inds)
        gt_boxes = np.empty((roidb[0]['boxes'].shape[0], 5), dtype=np.float32)
        gt_boxes[:, 0:4] = roidb[0]['boxes'][gt_inds, :]
        gt_boxes[:, 4] = roidb[0]['gt_classes'][gt_inds]
    else:
    ...
    if cfg.TRAIN.loss_frames == 3:
        if roidb[0]['bef_gt_classes'].size > 0:
            bef_gt_inds = np.where(roidb[0]['bef_gt_classes'] != 0)[0]
            bef_gt_boxes = np.empty((roidb[0]['bef_boxes'].shape[0], 5), dtype=np.float32)
            bef_gt_boxes[:, 0:4] = roidb[0]['bef_boxes'][bef_gt_inds, :]
            bef_gt_boxes[:, 4] = roidb[0]['bef_gt_classes'][bef_gt_inds]
        else:
            bef_gt_boxes = np.zeros((1, 5), dtype=np.float32)
            bef_gt_boxes[:, 4] = -1

        if roidb[0]['aft_gt_classes'].size > 0:
            aft_gt_inds = np.where(roidb[0]['aft_gt_classes'] != 0)[0]
            aft_gt_boxes = np.empty((roidb[0]['aft_boxes'].shape[0], 5), dtype=np.float32)
            aft_gt_boxes[:, 0:4] = roidb[0]['aft_boxes'][aft_gt_inds, :]
            aft_gt_boxes[:, 4] = roidb[0]['aft_gt_classes'][aft_gt_inds]
        else:
            aft_gt_boxes = np.zeros((1, 5), dtype=np.float32)
            aft_gt_boxes[:, 4] = -1
    ...
    if cfg.TRAIN.loss_frames == 3:
        label = {'gt_boxes': gt_boxes, 'bef_gt_boxes': bef_gt_boxes, 'aft_gt_boxes': aft_gt_boxes}
    else:
        label = {'gt_boxes': gt_boxes}
