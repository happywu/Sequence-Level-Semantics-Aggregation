...
class ImageNetVID(IMDB):
    ...
    def load_vid_annotation(self, iindex):
        ...
        if len(gt_classes):
            cls_name = np.argmax(np.bincount(gt_classes))
            cache_file_per_cls = os.path.join(self.cache_path, self.name + \
                                              '_' + self.classes[cls_name] + '_gt_roidb.pkl')
            roi_rec.update({'cls_db': cache_file_per_cls})
...
###################################################################################################
    ...
    def evaluate_detections_multiprocess_seqnms(self, detections, gpu_id):
        ...
        self.write_vid_results_multiprocess_gpuid(detections, gpu_id)
        return 1
    ...
    def evaluate_detections_multiprocess(self, detections):
        ...
        try:
            info_motion = self.do_python_eval_gen_motion()
            if info_motion:
                info += 'motion eval\n'
                info += info_motion
        except:
            print('eval motion error!')
        return info

    def get_result_file_template_gpuid(self, gpu_id):
        """
    ...
    def write_vid_results_multiprocess_gpuid(self, detection, gpu_id):
        """
        ...
        filename = self.get_result_file_template_gpuid(gpu_id).format('all')
        ...
        ############
        # sort by frame id
        sorted_idx = np.argsort(frame_ids)
        shuffled = True
        if (sorted_idx == np.arange(len(frame_ids))).all():
            shuffled = False
        ############
        start_idx = 0
        sum_frame_ids = np.cumsum(frame_seg_len)
        if shuffled:
            first_true_id = frame_ids[sorted_idx[0]]
        else:
        ...
        for im_ind in range(1, len(frame_ids)):
            t = time.time()
            if shuffled:
                true_id = frame_ids[sorted_idx[im_ind]]
            else:
                true_id = frame_ids[im_ind]
            video_index = np.searchsorted(sum_frame_ids, true_id) # get the video index
            if (video_index != start_video):  # reprensents a new video
                t1 = time.time()
                if shuffled:
                    video = [[all_boxes[j][i] for i in sorted_idx[start_idx:im_ind]]
                             for j in range(1, self.num_classes)] # take the last video
                else:
                    video = [all_boxes[j][start_idx:im_ind] for j in range(1, self.num_classes)] # take the last video
                dets_all = seq_nms(video)
                for j in xrange(1, self.num_classes):
                    for frame_ind, dets in enumerate(dets_all[j - 1]):
                        keep = nms(dets)
                        if shuffled:
                            all_boxes[j][sorted_idx[frame_ind + start_idx]] = dets[keep, :]
                        else:
            ...
            if (im_ind % 100 == 0):
                if shuffled:
                    print('{} seq_nms testing {} data {:.4f}s'.format(
                        frame_ids[sorted_idx[im_ind - 1]], im_ind, data_time / im_ind))
                else:
                    print('{} seq_nms testing {} data {:.4f}s'.format(
        ...
        # the last video
        if shuffled:
            video = [[all_boxes[j][i] for i in sorted_idx[start_idx:im_ind]] for j in range(1, self.num_classes)]
        else:
            ...
            for im_ind in range(len(frame_ids)):
                for cls_ind, cls in enumerate(self.classes):
                    if cls == '__background__':
                        continue
                    if shuffled:
                        dets = all_boxes[cls_ind][sorted_idx[im_ind]]
                    else:
                    ...
                    for k in range(dets.shape[0]):
                        if shuffled:
                            f.write('{:d} {:d} {:.4f} {:.2f} {:.2f} {:.2f} {:.2f}\n'.
                                    format(frame_ids[sorted_idx[im_ind]], cls_ind, dets[k, -1],
    ...
    def do_python_eval(self):
        ...
        filename = self.get_result_file_template().format('all')
        ap = vid_eval(False, filename, annopath, imageset_file, self.classes_map, annocache, ovthresh=0.5)
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
    ...
    def do_python_eval_gen_gpuid(self, gpu_number=None):
        """
        python evaluation wrapper
        :return: info_str
        """
        ...
        if gpu_number != None:
            ...
            for i in range(gpu_number):
                filename = self.get_result_file_template_gpuid(i).format('all')
    ...
    def do_python_eval_gen_motion(self, gpu_number=None):
        """
        python evaluation wrapper
        :return: info_str
        """
        ...
        ap = vid_eval_motion(multifiles, filenames, annopath, imageset_file, self.classes_map,
                             annocache, self.motion_iou_path,
