# --------------------------------------------------------
# low-Guided-Feature-Aggregation
...
def create_logger(root_output_path, log_path, cfg, image_set):
    # set up logger
    ...
    config_log_path = os.path.join(log_path, '{}'.format(cfg_name))
    if not os.path.exists(config_log_path):
        os.makedirs(config_log_path)

    final_log_path = os.path.join(config_log_path, time_str)
    if not os.path.exists(final_log_path):
        os.mkdir(final_log_path)

    return logger, final_output_path, final_log_path
